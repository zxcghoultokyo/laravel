...
## AI Agent Handling of Trigger Queries

Коли користувач натискає на тригер, `acceptTrigger()` відправляє спеціальне повідомлення в чат:
- **Product PDP**: `"Допоможіть з товаром \"Назва товару\""`
- **Category**: `"Покажи топ товари в категорії X"`
- **Exit-intent**: `"Допоможіть з товаром \"Товар на який дивився\""`

### Проблема з пасивними відповідями

**НЕПРАВИЛЬНО** (бот занадто пасивний):
```
Клієнт: Допоможіть з товаром "Комплект ECWCS Level 7"
Бот: Знайшов цей комплект. Уточни, що саме потрібно: 
     підібрати розмір, порівняти з аналогами?
```
❌ Клієнт з тригера вже зацікавлений — питання "що потрібно?" його втрачає!

**ПРАВИЛЬНО** (впевнена відповідь + CTA):
```
Клієнт: Допоможіть з товаром "Комплект ECWCS Level 7"
Бот: Оригінальний зимовий комплект армії США для -40°C — 
     куртка + штани з утеплювачем APEX.
     [Картка товару з детальним описом]
     Який розмір вам потрібен? Підкажіть зріст та вагу!
```
✅ Коротко про товар + показ деталей + конкретне питання про розмір

### Технічна реалізація

Обидва агенти (`FunctionCallingAgent` і `StreamingFunctionCallingAgent`) мають:

1. **detectTriggerQuery()** — детектує тригерні запити по фразам:
   - `"допоможіть з товаром"`
   - `"цікавить товар"`
   - `"покажи топ товари в категорії"`
   - `"хочу дізнатись більше про"`

2. **System message injection** — додає інструкцію для GPT:
   ```
   🚨 УВАГА: Це ТРИГЕРНИЙ ЗАПИТ!
   1. Знайди товар через search_products
   2. Дай КОРОТКУ але ВПЕВНЕНУ відповідь про товар
   3. Закінчи КОНКРЕТНИМ CTA залежно від типу товару
   ```

3. **generateTriggerOutro()** — ЗАВЖДИ генерує smart CTA для trigger queries:

   | Тип товару | CTA outro |
   |------------|-----------|
   | **Шолом** (category/title містить "шолом" або "helmet") | "Який обхват голови? Допоможу з підбором розміру шолома!" |
   | **Плитоноска** (category/title містить "плитоноска" або "plate carrier") | "Який розмір плит використовуєте? Допоможу з підбором!" |
   | **Одяг/взуття** (size_variants > 1) | "Який розмір вам потрібен? Підкажіть зріст та вагу — допоможу підібрати!" |
   | **Low stock** (quantity ≤ 3, інші категорії) | "Залишилось лише X шт. в наявності. Резервуємо?" |
   | **Default** (аксесуари без розмірів) | "Оформлюємо? Або є питання по характеристиках?" |

   **Пріоритет:** Type-based CTA (шолом/плитоноска) → Size-based (одяг) → Low stock → Default

### Response structure

```json
{
  "text": "Оригінальний зимовий комплект армії США для -40°C...",
  "data": {
    "products": [...],
    "messages": [
      {"type": "text", "content": "Intro про товар"},
      {"type": "product", "product": {...}, "comment": "..."},
      {"type": "text", "content": "CTA про розмір"}
    ]
  },
  "meta": {
    "outro": "Який розмір вам потрібен?",
    "is_trigger": true
  }
}
```
