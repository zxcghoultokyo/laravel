...
<script>
    ...
    // NEW: Render messages one by one with typing animation
    async function renderMessagesSequentially(messages) {
        const TYPING_DELAY = 300;  // Delay before each message
        const MESSAGE_DELAY = 150; // Delay between messages
        ...
        for (let i = 0; i < messages.length; i++) {
            ...
            if (msg.type === 'text') {
                appendMessage(msg.content, 'bot', i === 0); // Only save first
            } else if (msg.type === 'product') {
                // Show comment above product if exists
                if (msg.comment && msg.comment.trim()) {
                    const commentWrapper = document.createElement('div');
                    commentWrapper.className = 'flex justify-start message-appear mb-1';
                    commentWrapper.innerHTML = `
                        <div class="bg-slate-100 text-slate-600 rounded-xl px-3 py-2 text-sm max-w-[85%]">
                            ${msg.comment}
                        </div>
                    `;
                ...
                // Show product card
                const cardHtml = renderSingleProduct(msg.product);
        ...
        // Save all to history
        messageHistory.push({ type: 'messages', messages: messages });
        saveMessages(messageHistory);
    }
    ...
    function renderBotReply(payload) {
        ...
        // NEW: Sequential messages with typing animation
        if (payload.data?.messages?.length) {
            renderMessagesSequentially(payload.data.messages);
            return;
        }
    ...
    function restoreHistory() {
        ...
        saved.forEach(msg => {
            if (msg.type === 'text') {
                ...
                // Restore sequential messages (new format)
                msg.messages.forEach(m => {
                    if (m.type === 'text') {
                        appendMessage(m.content, 'bot', false);
                    } else if (m.type === 'product') {
                        if (m.comment && m.comment.trim()) {
                            const commentWrapper = document.createElement('div');
                            commentWrapper.className = 'flex justify-start mb-1';
                            commentWrapper.innerHTML = `
                                <div class="bg-slate-100 text-slate-600 rounded-xl px-3 py-2 text-sm max-w-[85%]">
                                    ${m.comment}
                                </div>
                        ...
                        const cardHtml = renderSingleProduct(m.product);
                        appendBotHtml(cardHtml, false);
                    }
                });
