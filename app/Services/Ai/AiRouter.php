...
- PRODUCT_SEARCH: пошук спорядження (плитоноска, броня, тактична куртка, берці) — ВКЛЮЧАЮЧИ запити з 'яку обрати', 'яка краще', 'що порадиш' якщо вказано категорію товару
...
- Видали ВСІ службові слова: 'допоможи', 'підібрати', 'мені', 'покажи', 'розкажи', 'про', 'знайди', 'потрібна', 'потрібен', 'хочу', 'шукаю', 'яку обрати', 'яка краще', 'що порадиш', 'а яку', 'яку'
- Залиш ТІЛЬКИ назву товару + характеристики (колір, розмір, бренд)
- 'а яку обрати плитоноску в пікселі' → 'плитоноска піксель'
- 'яка краще плитоноска чорна' → 'плитоноска чорна'
- 'допоможи підібрати шолом' → 'шолом'
- 'покажи чорні берці 43' → 'берці чорні 43'

Поверни JSON:
{
  \"intent\": \"PRODUCT_SEARCH | PRODUCT_COMPARISON | ORDER_STATUS | FAQ | SMALL_TALK | FALLBACK\",
  \"normalized_query\": \"очищені ключові слова\",
...
";
    ...
    protected function extractKeywordsFromMessage(string $message): string
    {
        ...
        $stopWords = [
            // Greetings
            'привіт', 'привет', 'hi', 'hello', 'добрий', 'день', 'вечір', 'ранок',
            // Helper words
