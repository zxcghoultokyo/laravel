...
class="p-4 sm:p-6 max-w-7xl mx-auto">
    ...
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
        <div>
            <h1 class="text-xl sm:text-2xl font-bold text-gray-900">📝 Промпт Пресети</h1>
            <p class="text-gray-600 mt-1 text-sm sm:text-base">Кастомні системні промпти з підтримкою змінних</p>
        </div>
        <div class="flex flex-col sm:flex-row gap-2">
            <button wire:click="generateForStore" 
                    wire:loading.attr="disabled"
                    wire:target="generateForStore"
                    class="bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white px-3 sm:px-4 py-2 rounded-lg flex items-center justify-center gap-2 disabled:opacity-50 text-sm sm:text-base">
                <svg wire:loading.remove wire:target="generateForStore" class="w-4 sm:w-5 h-4 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/>
                </svg>
                <svg wire:loading wire:target="generateForStore" class="w-4 sm:w-5 h-4 sm:h-5 animate-spin" fill="none" viewBox="0 0 24 24">
                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                </svg>
                <span wire:loading.remove wire:target="generateForStore" class="whitespace-nowrap">🤖 <span class="hidden sm:inline">Згенерувати для магазину</span><span class="sm:hidden">Генерувати</span></span>
                <span wire:loading wire:target="generateForStore">...</span>
            </button>
            <button wire:click="create" 
                    class="bg-blue-600 hover:bg-blue-700 text-white px-3 sm:px-4 py-2 rounded-lg flex items-center justify-center gap-2 text-sm sm:text-base">
                <svg class="w-4 sm:w-5 h-4 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
                </svg>
                <span class="whitespace-nowrap"><span class="hidden sm:inline">Новий</span> пресет</span>
            </button>
        </div>
    </div>
        ...
        @forelse($presets as $preset)
            <div class="bg-white rounded-lg shadow p-4">
                <div class="flex items-start justify-between mb-3">
                    <div>
                        ...
                        @if($preset->description)
                            <p class="text-sm text-gray-500 mt-1">{{ Str::limit($preset->description, 60) }}</p>
                        @endif
                    </div>
                    <button wire:click="toggleActive({{ $preset->id }})" 
                            class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors flex-shrink-0 {{ $preset->is_active ? 'bg-green-500' : 'bg-gray-300' }}">
                ...
                <div class="flex items-center justify-between">
                    <span class="text-xs text-gray-500">Пріоритет: {{ $preset->priority }}</span>
                    <div class="flex gap-1">
                        ...
                        <button wire:click="duplicate({{ $preset->id }})" 
                                class="p-2 text-gray-600 hover:text-purple-600 hover:bg-purple-50 rounded" title="Дублювати">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase hidden md:table-cell">Змінні</th>
                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Пріор.</th>
    ...
    {{-- Create/Edit Modal --}}
    @if($showModal)
        <div class="fixed inset-0 bg-black/50 z-40 flex items-start justify-center pt-4 sm:pt-10 overflow-y-auto px-2 sm:px-4">
            <div class="bg-white rounded-lg shadow-xl w-full max-w-4xl mx-0 sm:mx-4 my-4 sm:my-8" @click.outside="$wire.showModal = false">
                <div class="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-200 sticky top-0 bg-white z-10 rounded-t-lg">
                ...
                <div class="p-4 sm:p-6 space-y-4 sm:space-y-6 max-h-[75vh] overflow-y-auto">
                    {{-- Basic Info --}}
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Назва *</label>
                            <input type="text" wire:model="name" 
                                   class="w-full rounded-lg border-gray-300 text-sm sm:text-base"
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
                            <div>
                                <label class="block text-xs text-gray-500 mb-1 flex items-center gap-1">
                                    Мова
                                    <span class="text-gray-400 cursor-help" title="Мова клієнта: uk, en, ru">ⓘ</span>
                                </label>
                            ...
                            <div>
                                <label class="block text-xs text-gray-500 mb-1 flex items-center gap-1">
                                    Тон
                                    <span class="text-gray-400 cursor-help" title="Офіційний/Спартанський/Дружній">ⓘ</span>
                                </label>
                            ...
                            <div>
                                <label class="block text-xs text-gray-500 mb-1 flex items-center gap-1">
                                    UTM
                                    <span class="text-gray-400 cursor-help" title="utm_campaign для акцій">ⓘ</span>
                                </label>
                            ...
                            <div>
                                <label class="block text-xs text-gray-500 mb-1 flex items-center gap-1">
                                    Тип
                                    <span class="text-gray-400 cursor-help" title="Тип магазину">ⓘ</span>
                                </label>
                                <select wire:model="storeType"
                                        class="w-full rounded-lg border-gray-300 text-sm">
                                    @foreach($storeTypes as $code => $label)
                                        <option value="{{ $code }}">{{ $label }}</option>
                                    @endforeach
                                </select>
                            </div>
                ...
                <div class="flex flex-col-reverse sm:flex-row justify-end gap-2 sm:gap-3 px-4 sm:px-6 py-3 sm:py-4 border-t border-gray-200 bg-gray-50 rounded-b-lg sticky bottom-0">
                    <button wire:click="$set('showModal', false)" 
                            class="w-full sm:w-auto px-4 py-2 text-gray-700 hover:bg-gray-200 rounded-lg text-center">
                        Скасувати
                    </button>
                    <button wire:click="save" 
                            class="w-full sm:w-auto px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-center">
    ...
    {{-- Test Modal --}}
    @if($showTestModal)
        <div class="fixed inset-0 bg-black/50 z-40 flex items-center justify-center px-2 sm:px-4">
            <div class="bg-white rounded-lg shadow-xl w-full max-w-2xl mx-0 sm:mx-4" @click.outside="$wire.showTestModal = false">
                <div class="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-200">
                    <h2 class="text-lg sm:text-xl font-semibold text-gray-900">🧪 Тест пресету</h2>
                    <button wire:click="$set('showTestModal', false)" class="text-gray-400 hover:text-gray-600 p-1">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                        </svg>
                    </button>
