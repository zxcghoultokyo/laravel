...
class ProductDetailsTool
{
    ...
    public function getCards(array $ids, int $limit = 10, ?int $tenantId = null): array
    {
        ...
        $cards = $orderedProducts->map(function ($product) use ($parentRawMap, $variantsMap) {
        ...
        return $this->attachMarketingHints($cards, $productTenantId);
    }

    /**
     * Attach tenant marketing hints (from knowledge base) to each card by article.
     * Silent no-op on any failure so chat flow never breaks.
     *
     * @param  array<int,array<string,mixed>>  $cards
     * @return array<int,array<string,mixed>>
     */
    protected function attachMarketingHints(array $cards, ?int $tenantId): array
    {
        if ($cards === [] || ! $tenantId) {
            return $cards;
        }
        ...
        foreach ($cards as $card) {
            if (! empty($card['article'])) {
                $articles[] = (string) $card['article'];
            }
            if (! empty($card['parent_article'])) {
                $articles[] = (string) $card['parent_article'];
            }
        }
        ...
        try {
            $hints = app(KnowledgeBaseService::class)->getHintsForArticles($articles, $tenantId);
        } catch (\Throwable $e) {
            Log::warning('ProductDetailsTool: hint lookup failed', ['error' => $e->getMessage()]);
        ...
        foreach ($cards as &$card) {
            $hint = $hints[$card['article'] ?? ''] ?? $hints[$card['parent_article'] ?? ''] ?? null;
            if ($hint) {
                $card['marketing_hint'] = $hint;
            }
        }
