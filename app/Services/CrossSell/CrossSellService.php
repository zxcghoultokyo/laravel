...
class CrossSellService
{
     ...
     * NOTE: These are generic rules for common product types.
     * For niche-specific rules, use cross_sell_rules DB table per tenant.
     */
    protected array $fallbackRules = [
        // Generic clothing/fashion
        'boots' => ['шкарпетки', 'устілки', 'засоби догляду'],
        'shoes' => ['шкарпетки', 'устілки', 'засоби догляду'],
        'взуття' => ['шкарпетки', 'устілки', 'засоби догляду'],
        'gloves' => ['шапки', 'шарфи', 'аксесуари'],
        'рукавиці' => ['шапки', 'шарфи', 'аксесуари'],
        'backpacks' => ['сумки', 'аксесуари', 'гаманці'],
        'рюкзаки' => ['сумки', 'аксесуари', 'гаманці'],
        'pants' => ['ремені', 'шкарпетки', 'взуття'],
        'штани' => ['ремені', 'шкарпетки', 'взуття'],
        'jackets' => ['светри', 'шарфи', 'шапки'],
        'куртки' => ['светри', 'шарфи', 'шапки'],
        'dresses' => ['сумки', 'прикраси', 'взуття'],
        'сукні' => ['сумки', 'прикраси', 'взуття'],
    ];
    ...
    protected function askAiForAccessories(Product $product): array
    {
        try {
            ...
            $prompt = sprintf(
                "Користувач переглядає товар:\nКатегорія: %s\nНазва: %s\n\nВибери 2-3 категорії СУПУТНІХ ТОВАРІВ які б логічно доповнили цю покупку.\n\nПРАВИЛА:\n- Вибирай РЕЛЕВАНТНІ товари з тієї ж сфери використання\n- Пропонуй аксесуари та доповнення, а не схожі товари\n- Для одягу: аксесуари (ремені, шарфи, шапки), взуття, сумки\n- Для взуття: шкарпетки, устілки, засоби догляду\n- Для сумок/рюкзаків: гаманці, органайзери, аксесуари\n- НЕ пропонуй товари тієї ж категорії!\n\nДоступні категорії магазину:\n%s\n\nВідповідь ТІЛЬКИ JSON масивом: [{\"category\": \"точна назва категорії\", \"reason\": \"коротке пояснення українською\"}]",
