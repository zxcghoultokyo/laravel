...
## 🚀 Автоматичний Онбординг (Поточний стан)

При створенні тенанта через адмін панель (`/admin/tenants`) автоматично:

1. ✅ Створюється запис в `tenants`
2. ✅ Створюється `widget_settings` (з дефолтними налаштуваннями)
3. ✅ Прив'язується користувач до тенанта

### Що НЕ робиться автоматично (треба виправити!):

| Дія | Поточний стан | Потрібно |
|-----|--------------|----------|
| Sync продуктів з Horoshop | ❌ Тільки по schedule | ✅ При онбордингу |
| AI Enrichment | ❌ Тільки по schedule | ✅ Після sync |
| Rebuild категорій | ❌ Тільки по schedule | ✅ Після sync |
| Meili індексація | ❌ Тільки по schedule | ✅ Після enrichment |
| Sync брендів | ❌ Тільки по schedule | ✅ Після sync |
