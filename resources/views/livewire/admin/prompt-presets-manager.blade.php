...
class="p-6 max-w-7xl mx-auto">
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">📝 Промпт Пресети</h1>
            <p class="text-gray-600 dark:text-gray-400 mt-1">Кастомні системні промпти з підтримкою змінних</p>
        </div>
    ...
    <div class="bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-800 rounded-lg p-4 mb-6">
        <div class="flex items-start gap-3">
            ...
            <div class="text-sm text-blue-800 dark:text-blue-200">
                <p><strong>Змінні:</strong> Використовуйте <code class="bg-blue-100 dark:bg-blue-800 px-1 rounded">&#123;&#123;variable_name&#125;&#125;</code> для динамічних значень.</p>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead class="bg-gray-50 dark:bg-gray-700">
                <tr>
                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Назва</th>
                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Умови</th>
                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Змінні</th>
                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Пріоритет</th>
                    <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Статус</th>
                    <th class="px-4 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Дії</th>
                </tr>
            </thead>
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                        <td class="px-4 py-4">
                            ...
                            @if($preset->description)
                                <p class="text-sm text-gray-500 dark:text-gray-400 mt-1 truncate max-w-xs">{{ $preset->description }}</p>
                            @endif
                        </td>
                        <td class="px-4 py-4">
                            ...
                            @if(!empty($preset->variables))
                                <div class="flex flex-wrap gap-1">
                                    @foreach(array_slice($preset->variables, 0, 3) as $var)
                                        <span class="px-2 py-0.5 text-xs bg-gray-100 dark:bg-gray-600 text-gray-700 dark:text-gray-300 rounded font-mono">@{{ '{{' . $var['name'] . '}}' }}</span>
                                    @endforeach
                        ...
                        <td class="px-4 py-4">
                            <span class="px-2 py-1 text-sm bg-gray-100 dark:bg-gray-600 rounded">{{ $preset->priority }}</span>
                        </td>
                        <td class="px-4 py-4">
    ...
    @if($showModal)
        <div class="fixed inset-0 bg-black/50 z-40 flex items-start justify-center pt-10 overflow-y-auto">
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-4xl mx-4 my-8" @click.outside="$wire.showModal = false">
                ...
                <div class="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-1">
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Системний промпт *</label>
                    ...
                    {{-- Variables --}}
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Змінні</label>
                        <div class="space-y-2">
                            @foreach($variables as $index => $var)
                                <div class="flex items-center gap-2 p-2 bg-gray-50 dark:bg-gray-700 rounded">
                                    <code class="text-sm text-purple-600 dark:text-purple-400">@verbatim{{@endverbatim{{ $var['name'] }}@verbatim}}@endverbatim</code>
                                    <span class="text-gray-400">=</span>
                                    <input type="text" wire:model="variables.{{ $index }}.default"
                                           class="flex-1 text-sm rounded border-gray-300 dark:border-gray-600 dark:bg-gray-600 dark:text-white"
                        ...
                        <div class="flex gap-2 mt-2">
                            ...
                            <button wire:click="addVariable" type="button"
                                    class="px-3 py-2 bg-gray-100 hover:bg-gray-200 dark:bg-gray-600 dark:hover:bg-gray-500 rounded-lg text-sm">
                                Додати
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="border-t border-gray-200 dark:border-gray-700 pt-4">
                        <h3 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Умови застосування</h3>
                    ...
                    <div class="flex items-center gap-6 pt-4 border-t border-gray-200 dark:border-gray-700">
                        <label class="flex items-center gap-2 cursor-pointer">
                            ...
                            <span class="text-sm text-gray-700 dark:text-gray-300">Активний</span>
                        </label>
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" wire:model="is_default" class="rounded border-gray-300 text-blue-600">
                            <span class="text-sm text-gray-700 dark:text-gray-300">За замовчуванням</span>
                        </label>
                    </div>
                </div>

                <div class="flex justify-end gap-3 px-6 py-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700 rounded-b-lg">
                    <button wire:click="$set('showModal', false)" 
                            class="px-4 py-2 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600 rounded-lg">
                        Скасувати
                    </button>
    ...
    @if($showTestModal)
        <div class="fixed inset-0 bg-black/50 z-40 flex items-center justify-center">
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-2xl mx-4" @click.outside="$wire.showTestModal = false">
                <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                    <h2 class="text-xl font-semibold text-gray-900 dark:text-white">🧪 Тест пресету</h2>
                ...
                <div class="p-6 space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Тестове повідомлення</label>
                        <div class="flex gap-2">
                    ...
                    @if($testResponse)
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Відповідь AI</label>
                            <div class="p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                                <p class="text-gray-800 dark:text-gray-200 whitespace-pre-wrap">{{ $testResponse }}</p>
                            </div>
                        </div>
                    @endif
