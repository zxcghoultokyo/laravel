...
### 7. Detail Summarization (Follow‑up "розкажи про …")
**Файли**: 
- [app/Services/Agent/AgentOrchestrator.php](../../app/Services/Agent/AgentOrchestrator.php)
- [app/Services/Agent/Tools/ProductDetailsTool.php](../../app/Services/Agent/Tools/ProductDetailsTool.php)

**Роль**: Відповідати короткою фактичною вижимкою по конкретному товару за запитами типу: "розкажи про плитоноску Схід 24".

**Деталі реалізації**:
- Виявлення follow‑up: `isDetailsFollowUp(message)` шукає патерни на кшталт "розкажи про", "покажи деталі".
- Виділення назви: `extractSearchTermsFromMessage()` прибирає службові слова і лишає токени назви/моделі.
- Пошук за назвою: Meilisearch (або Eloquent fallback) без додаткових фільтрів, з умовою `in_stock=true`.
- Перевірка відповідності: `calculateTitleMatchConfidence()` знижує вагу загальних слів (типу "плитоноска"), включає точний збіг підрядка; поріг ~60%.
- Побудова відповіді: `buildProductDetailMessage(product)` формує коротку вижимку з перевірених полів.

**Джерела фактів (тільки перевірені поля):**
- `title`, `price/price_old`, `category_path`, `brand`
- `characteristics` (атрибути; зливка з поточного та батьківського товару)
- `description` (парсинг з `raw` через `ProductRawExtractor`)

**Форматування відповіді**:
- Якщо AI доступний: строго інструктований промпт (низька температура) конденсує опис БЕЗ галюцинацій. Заборонено вигадані властивості; дозволені перефразування лише на основі вхідних полів.
- Fallback (без AI): структуровані буліти — 3–6 пунктів, коротко: призначення, основні характеристики (матеріал/сумісність/вага якщо є), камо/колір, актуальна ціна.

**Приклад запиту**:
"розкажи про плитоноску Схід 24"

**Очікувана відповідь (приклад)**:
"Плитоноска Схід 24: легка модульна система для плит стандарту SAPI. Матеріал — кордура 500D; сумісність із MOLLE; регулювання підросту. Доступний камуфляж: піксель. Ціна — 12 000 грн (актуальна). Деталі: швидке скидання; підсумки сумісні (за потреби додаються окремо)."

Логування: відсоток відповідності назві, вибраний товар, використаний режим форматування (AI чи fallback).
