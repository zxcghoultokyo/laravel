...
class ChatController extends Controller
{
    ...
    public function handle(Request $request)
    {
        ...
        $confidence       = $routing['confidence']       ?? 0.0;

        // Якщо AI взагалі не зміг відпрацювати → fallback на старий пайплайн
        if ($intent === 'FALLBACK') {
        ...
        if ($intent === 'ORDER_STATUS') {
            ...
            // AI каже: це про замовлення, але без номера
        ...
        if ($intent === 'FAQ') {
            ...
            // Якщо FAQ-сервіс не знайшов — падаємо далі в fallback-пайплайн
            return $this->fallbackPipeline($message, $messageLower);
        }
        ...
        if ($intent === 'PRODUCT_SEARCH') {
            ...
            // Якщо товарів не знайшли — даємо шанс AiRecommender
            try {
        ...
        // 4) SMALL_TALK — можна відповісти чимось простим або через AI
        ...
        // 5) UNKNOWN — нічого зрозумілого, йдемо в fallback-пайплайн
        return $this->fallbackPipeline($message, $messageLower);
    }

    /**
     * Старий пайплайн: FAQ -> товари -> AI -> no_results
     */
    ...
    /**
     * Робота з замовленнями (як ми вже робили раніше).
     */
