...
- НІКОЛИ не пропонуй як самостійний подарунок ці типи товарів (це аксесуари/складові або утилітарні речі, не подарунки):
  фартух, нарукавники, дошка, ніж, качалка, ложки окремо, совок, віник, швабра, пакет/пакування/коробка/листівка, таця для триступеневих карток окремо, підвіски для тренажеру, тренажер-перекладина, коробочка постійності (актуальна лише 0-12 місяців).
...
TXT;
        ...
        $ageRetryAddition = <<<'TXT'

AGE-RETRY (обов'язкове правило повторного пошуку):
- Якщо клієнт у відповідь на підбірку каже "пізно/рано/малий/великий/замалий/завеликий/не підходить по віку" — ОБОВ'ЯЗКОВО повторно виклич search_products з ЧІТКИМ фільтром по віку (`min_age_months` та/або `max_age_months` у місяцях).
- Приклад: клієнт сказав "це рано, дитині вже 1 рік" → повторний запит має містити `min_age_months=12` (не показуй товари 0-6м, 0-12м).
- Приклад: клієнт сказав "це пізно, дитині 6 місяців" → `max_age_months=12`.
- У повторному пошуку ЗАБОРОНЕНО повторювати вже показані артикули (використовуй exclude_articles).
- Якщо користувач згадав "рік / 1 рочок / на годик" — це `min_age_months=12`, `max_age_months=36`.
TXT;
        ...
        if (! str_contains($baseSysPrompt, 'ПОДАРУНОК-БЕЗПЕКА')) {
        ...
        } elseif (! str_contains($baseSysPrompt, 'нарукавники') || ! str_contains($baseSysPrompt, 'коробочка постійності')) {
            // Старий блок — оновити з розширеним списком
            $baseSysPrompt = preg_replace(
                '/\nПОДАРУНОК-БЕЗПЕКА.*?(?=\n[А-ЯA-Z][А-ЯA-Z-]+:|\z)/us',
                trim($baseAddition)."\n",
                $baseSysPrompt,
                1
            ...
            $baseChanged = true;
            $this->line('[base] оновлено блок ПОДАРУНОК-БЕЗПЕКА (розширений список)');
        } else {
            $this->line('[base] блок ПОДАРУНОК-БЕЗПЕКА вже актуальний');
        }

        if (! str_contains($baseSysPrompt, 'AGE-RETRY')) {
            $baseSysPrompt = rtrim($baseSysPrompt)."\n".$ageRetryAddition."\n";
            $baseChanged = true;
            $this->line('[base] додаю блок AGE-RETRY');
        } else {
            $this->line('[base] блок AGE-RETRY вже присутній');
        }
