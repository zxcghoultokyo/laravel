...
- Використай синоніми/ширший запит: "тактичний ліхтар" → search_products("ліхтар OR ліхтарик OR flashlight")
- Розбий складний запит: "зимова мембранна куртка" → search_products("куртка зимова OR куртка мембрана")
- Спробуй ключове слово: "захист для ніг" → search_products("наколінники OR щитки OR захист")
...
📅 СЕЗОННІСТЬ:
{$seasonNote}
- Сезонні запити ("що беруть влітку", "що потрібно навесні") → шукай КОНКРЕТНІ категорії товарів!
  - Літо → search_products("футболка OR панама OR шорти OR кепка OR окуляри OR легкий")
  - Зима → search_products("куртка зимова OR термобілизна OR рукавиці OR шапка OR флісова")
  - Весна/Осінь → search_products("куртка softshell OR штани OR дощовик OR демісезонний")
- НЕ шукай абстрактні слова ("дихаючий", "вентиляція") — шукай НАЗВИ товарів!
...
Вікові категорії:
- 0-11 місяців → category="МАЛЮКАМ 0 – 1"
- 1-2 роки → category="ТОДЛЕРАМ 1 – 3"
- 3-6 років → category="ДОШКІЛЬНЯТАМ 3 – 7"
- 7+ років → category="ШКОЛЯРАМ"

Приклади:
- "для немовляти" / "6 місяців" → category="МАЛЮКАМ 0 – 1"
- "для дитини 1 рік" / "1.5 роки" / "2 роки" → category="ТОДЛЕРАМ 1 – 3"
- "3 роки" / "4 роки" / "5 років" / "6 років" → category="ДОШКІЛЬНЯТАМ 3 – 7"
- "7 років" / "8 років" / "для школяра" → category="ШКОЛЯРАМ"
БЕЗ фільтра category — пошук поверне товари БУДЬ-ЯКОГО віку, що НЕПРАВИЛЬНО!

⚠️ ВАЖЛИВО: СПОЧАТКУ ШУКАЙ — ПОТІМ УТОЧНЮЙ!
Якщо вік НЕ вказано — СПОЧАТКУ search_products() БЕЗ category фільтра, покажи товари, а ПОТІМ запитай вік!
❌ ЗАБОРОНЕНО: "Для якого віку шукаєте?" без показу товарів
✅ ПРАВИЛЬНО: search_products("конструктор") → покажи 3 товари → "Яка вікова категорія? Підберу точніше!"
Коли запит містить КОНКРЕТНУ назву товару (наприклад "набір юного хіміка", "пазл сонячна система") — ЗАВЖДИ шукай саме за цією назвою БЕЗ вікового фільтра!
AGE;
        } else {
            $searchModule .= <<<'NOAGE'

⛔ НЕ питай про вік дитини! Цей магазин НЕ є дитячим.
Якщо запит загальний ("що на весну", "порекомендуй") — одразу шукай товари через search_products(), не уточнюй вік.
NOAGE;
        }
...
3. CTA: "Який розмір? Підкажу!" або "Є питання? Допоможу!"
